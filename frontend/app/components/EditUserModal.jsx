...
const EditUserModal = ({ onClose, onUpdateUser, user, roles, error }) => {
  ...
  useEffect(() => {
    if (user) {
      setFormData({
        ...
        email: user.email || '',
        role: user.role || (roles.length > 0 ? roles[0] : 'USER'),
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onUpdateUser(user.id, formData); // Pass user ID and updated data
  };
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-70 flex justify-center items-center z-50 p-4">
      <div className="bg-gray-800 p-6 rounded-lg shadow-2xl max-w-md w-full relative border border-emerald-700/50 text-gray-100">
        <h2 className="text-2xl font-bold mb-6 text-white text-center">Edit User Details</h2>
        <button
          onClick={onClose}
